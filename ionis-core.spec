...
# PostgreSQL (PG-1) DDL -- kept apart from the ClickHouse ddl/ so ionis-db-init, which applies
# ddl/*.sql to ClickHouse, can never pick one up. The ADIF tier is generated by adif_tier.py
# from ADIF's published all.json, verified against the pinned adif.org checksums below.
install -d %{buildroot}%{_datadir}/%{name}/pg
install -m 644 src/pg/*.sql %{buildroot}%{_datadir}/%{name}/pg/
install -m 755 scripts/adif_tier.py %{buildroot}%{_datadir}/%{name}/scripts/
install -m 644 data/adif_upstream_sha256.json %{buildroot}%{_datadir}/%{name}/data/
...
%{_datadir}/%{name}/data/adif_upstream_sha256.json
%dir %{_datadir}/%{name}/pg
...
- ADIF reference tier for PG-1 (the one `ionis` database; Judge 2026-09-26). New
  ...
  published all.json, refusing any file whose SHA-256 differs from adif.org's (pins in
  data/adif_upstream_sha256.json, same values as adif-mcp 1.1.2). Tables: adif.release,
  adif.datatype (28), adif.field (186), and one table per ADIF enumeration (25; 3,345
  records for 3.1.7, 3,340 for 3.1.6). Keys carry adif_version, so versions coexist and a
  release is never replaced; enumeration rows are keyed by ADIF's own record key, with a
  unique natural key wherever ADIF's data has one; every row keeps the record as published
