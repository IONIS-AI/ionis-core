...
| `solar.ssn_bronze` | 76.21K | SIDC Brussels sunspot number archive, 1818–. **One row per line, all 8 columns** (incl. DecimalDate), line number and raw line; -1 → NULL. Exact copy per run via staging + EXCHANGE TABLES. Audit: `verify_ssn_ingest.sh` | `solar-ssn-download` + `solar-ssn-ingest` | `44-solar_ssn_bronze.sql` |
...
### 4a. The ADIF reference tier (PostgreSQL, PG-1 database `ionis`, schema `adif`)

Not in ClickHouse. ADIF's own definitions, loaded from ADIF's published `all.json` and never edited here. ClickHouse will read them only through dictionaries (per the data spec). Generated by `scripts/adif_tier.py`, which refuses a file whose SHA-256 differs from adif.org's (`data/adif_upstream_sha256.json`).
...
Versions coexist (3.1.6 and 3.1.7) and are never replaced. `Country` is not here: ADIF names it but publishes no table, and its names are `adif.dxcc_entity_code.entity_name`. `primary_administrative_subdivision` and `region` have no unique code (codes are reused after deletion, and Kosovo's region changes entity over time), so reference them by `record_key`.
